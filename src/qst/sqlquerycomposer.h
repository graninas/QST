...
#ifndef SQLQUERYCOMPOSER_H
#define SQLQUERYCOMPOSER_H
...
public:
	...
	void addSelectClauseField(const SqlField &field);
	void addStuffField(const SqlField &field);

	QString query(const SqlQueryTypes &sqlType = sql_select) const;
...
private:
	...
	void	_addCondition(const QString &field, const SqlValue &value);

	void	_addField(const QString &field);

	void	_addInsertValue(const QString &field, const SqlValue &value);
	void	_addUpdateValue(const QString &field, const SqlValue &value);
	void	_addParameter(const SqlValue &value);
	...
	_StringTemplate _functorToStringTemplate(const Functor &functor) const;
